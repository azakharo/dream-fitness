...
**Swagger/OpenAPI** — автогенерируемая интерактивная документация REST API.

### 12.2. Реализация

| Компонент  | Решение                   |
| ---------- | ------------------------- |
| Библиотека | @nestjs/swagger           |
| Размещение | На уровне API Gateway     |
| Endpoint   | /api/docs                 |
| Auth       | Bearer token в Swagger UI |

### 12.3. Структура документации

Все микросервисы агрегируются через API Gateway:

- API Gateway собирает схемы всех сервисов
- Единая точка входа для тестирования endpoints
...
// В API Gateway — агрегация всех схем
```

### 12.5. Security

| Environment | Доступность                      |
| ----------- | -------------------------------- |
| Development | Полный доступ без ограничений    |
| Production  | Ограниченный доступ или отключён |
